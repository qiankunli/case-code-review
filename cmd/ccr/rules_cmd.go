...
func printRulesUsage() {
	fmt.Println(`Usage:
  ccr rules <sub-command>

Sub-commands:
  check <file>   Show which review rule applies to a given file path

Use "ccr rules check -h" for more information.`)
}

func printRulesCheckUsage() {
	fmt.Println(`Usage:
  ccr rules check [flags] <file-path>
...
Examples:
  ccr rules check src/main/java/com/example/Foo.java
  ccr rules check --rule custom.json src/main/resources/mapper/UserMapper.xml`)
}
