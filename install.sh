#!/bin/sh
# Install the ccr (Open Code Review) CLI from GitHub releases.
#   curl -fsSL https://raw.githubusercontent.com/qiankunli/case-code-review/main/install.sh | sh
# Env: CCR_INSTALL_DIR (default /usr/local/bin), CCR_VERSION (default latest).
set -eu

main() {
  REPO="qiankunli/case-code-review"
  BIN="ccr"
  ASSET_PREFIX="casecodereview"
  INSTALL_DIR="${CCR_INSTALL_DIR:-/usr/local/bin}"
  VERSION="${CCR_VERSION:-}"
...
install_binary() {
  ...
  if mkdir -p "$dir" 2>/dev/null && [ -w "$dir" ]; then
    ...
    err "$dir is not writable and sudo is unavailable; set CCR_INSTALL_DIR to a writable path"
  fi
}
