...
- Context 模型：unit 拿到后怎么收评审素材——`Marker`(spec/case/rule/link/doc) / `MarkerFinder`(符号→marker) / `Relation`(self/owner/caller/callee/used) / `Clue`(组合)，两轴正交——`docs/context-model.md`
