...
8. **待补概念：unit 生命周期（完成边界）**——`RunPerFile` 返回 ≠ unit 审完：异步 comment 后处理（relocation）落定才是，而这个"完成时刻"目前没有名字，靠编排顺序维持（debrief 队列在 `Await()` 后 flush；见 #90 竞态的教训）。已有三个补丁属于同一缺口；当第三个"loop 结束后消费 unit 结果"的需求出现（如碰头会），应升级为显式生命周期（scope `Close()` 自产 debrief、向已关闭 scope 写入可检测）。
