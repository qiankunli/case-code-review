...
// TypeScript backend with the same low-confidence repository-index shape. It uses
...
// scanTypeScriptRepository extracts defs/refs from TypeScript, JavaScript, TSX, and JSX files.
...
func scanTypeScriptRepository(repoDir string) *RepositoryIndex {
	ex, _ := scanTS(repoDir)
	return ex
}

func scanTS(repoDir string) (*RepositoryIndex, error) {
