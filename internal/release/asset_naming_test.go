...
func TestReleaseYmlMatchesURLPattern(t *testing.T) {
	...
	// Match: BIN_NAME=ccr-${{ matrix.goos }}-${{ matrix.goarch }}
	re := regexp.MustCompile(`BIN_NAME=(ccr[^\n]*\}\})`)
...
func TestMakefileBuildMatchesURLPattern(t *testing.T) {
	...
	for _, p := range platforms {
		...
		rendered = strings.ReplaceAll(rendered, "$(BINARY_NAME)", "ccr")
...
func TestURLPatternHTTPS(t *testing.T) {
	pkg := loadPackageJSON(t)
	if !strings.HasPrefix(pkg.OcrConfig.URLPattern, "https://github.com/qiankunli/case-code-review/releases/download/") {
