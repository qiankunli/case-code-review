...
// Python backend, same shape as ScanGo: defs with one-line signatures plus
...
// Skip rules mirror ScanGo: hidden dirs, vendor trees, tests, oversized files.
const pyScanScript = `
...
// ScanPy extracts defs/refs from the repo's Python files. Returns nil when
// python3 is unavailable or anything goes wrong — callers merge backends
// and a missing one just narrows the map.
func ScanPy(repoDir string) *Extraction {
...
// Scan runs every language backend and merges the results — the single
// entry point map consumers should use. Backends are independent; each may
...
func Scan(repoDir string) *Extraction {
	...
	for _, ex := range []*Extraction{ScanGo(repoDir), ScanPy(repoDir)} {
		...
		for f, ds := range ex.Defs {
			merged.Defs[f] = append(merged.Defs[f], ds...)
		}
		for f, rs := range ex.Refs {
			if merged.Refs[f] == nil {
				merged.Refs[f] = rs
				continue
			}
			for k, v := range rs {
