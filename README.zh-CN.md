...
**2. 按 *unit* 触发 review loop，中间有一步归并。** unit 是一个 review loop 的作用域。ocr 只按**文件**评审；ccr 把粒度做成 **unit**，是一条阶梯：**函数 → 类 → 文件 → 模块/文件夹**。ccr 先从 diff 收集到改动的 unit（函数），再按**归并策略**把它们合并成真正触发 loop 的 unit，并随改动变大**沿阶梯往上爬**：一个文件改了很多函数 → 归并成一个文件 unit；改动横扫一片 → 归并到文件级、甚至目录级 unit。于是小改动聚焦最高、大改动成本有界——一个归并后的 unit 一个 loop。
