...
2. **找的是需要背景才找得到的那种 bug。** ccr *就是*在找 bug——只是真正要紧的不是局部语法（那是 lint 的活），而是"一次改动悄悄破坏了某个 caller 的假设、或违反了 diff 看不见的不变量"。这种 bug 得有背景才找得到。契约（spec/case）把问题变具体——*这次改动还满足函数必须保证的东西吗？*——于是是**对着真契约逐条核对**，而不是漫无边际地猜。
