...
评审好不好,盯三个追求(互相拉扯,不能只优化一个):

1. **健壮性**——每个 unit 的 loop 真跑完:大文件不因预算被跳过、长链不被超时截断。截断的沉默最危险,因为它长得像"没问题"。
2. **准确性**——真问题找得到:不空转交 clean,也不编造 finding。
3. **成本**——时间与 token 有界:review 跟不上开发节奏,再准也会被绕过。

落地手段都围着 **review loop** 这一个核心:

1. **loop 本身的能力**:工具面(搜索/读文件/call graph)、记忆机制(压缩、超时 wrap-up)。
2. **loop 的粒度**:file → unit,贴近改动的真实粒度——一次改动通常是一个需求及其相关文件,评审作用域应当与之对齐(函数 → 调用链 → 文件的归并阶梯)。
3. **loop 的 context**:分两类——ccr **自己能捕获的**(现场解析函数边界、grep 使用点、call graph 邻域、源码 doc 抽取),追求零采纳成本;**外部输入的**(authored spec/case/rule/link、上轮评审 history、需求背景),追求高信号。

量测对应 `eval/README.md`:质量轴测准确性,效率轴测成本,截断/超时/未收尾信号测健壮性。
